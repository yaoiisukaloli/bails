...
const makeMessagesSocket = config => {
    ...
    const sendReceipt = async (jid, participant, messageIds, type) => {
        ...
        if (type) {
            node.attrs.type = (0, WABinary_1.isJidNewsLetter)(jid)
                ? "read-self"
                : type;
        }
    ...
    const relayMessage = async (
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                // Message edit
                if (
                    (_a = message.protocolMessage) === null || _a === void 0
                        ? void 0
                        : _a.editedMessage
                ) {
                    msgId =
                        (_b = message.protocolMessage.key) === null ||
                        _b === void 0
                            ? void 0
                            : _b.id;
                ...
                // Message delete
                if (
                    ((_c = message.protocolMessage) === null || _c === void 0
                        ? void 0
                        : _c.type) ===
                    WAProto_1.proto.Message.ProtocolMessage.Type.REVOKE
                ) {
                    msgId =
                        (_d = message.protocolMessage.key) === null ||
                        _d === void 0
                            ? void 0
                            : _d.id;
                    message = {};
                ...
                const { user: meUser, device: meDevice } = (0,
                ...
                for (const { user, device } of devices) {
                    ...
                    const jid = (0, WABinary_1.jidEncode)(
                        isMe && isLid
                            ? ((_f =
                                  (_e = authState.creds) === null ||
                                  _e === void 0
                                      ? void 0
                                      : _e.me) === null || _f === void 0
                                  ? void 0
                                  : _f.lid.split(":")[0]) || user
                            : user,
                        isLid ? "lid" : "s.whatsapp.net",
            ...
            if (!isNewsletter && buttonType) {
                ...
                const filteredNode =
                    WABinary_1.getBinaryNodeFilter(additionalNodes);
    ...
    return {
        ...
        /* sendMessage: async (jid, content, options = {}) => {
            ...
            const getParticipantAttr = () =>
                filter ? { participant: { jid } } : {};
            const messageType = rahmi.detectType(content);

            if (messageType) {
                switch (messageType) {
                    case "PAYMENT":
                        const paymentContent = await rahmi.handlePayment(
                            content,
                            quoted
                        );
                    ...
                    case "PRODUCT":
                        const productContent = await rahmi.handleProduct(
                            content,
                            jid,
                            quoted
                        );
                        const productMsg =
                            await Utils_1.generateWAMessageFromContent(
                                jid,
                                productContent,
                                { quoted }
                            );
                    ...
                    case "INTERACTIVE":
                        const interactiveContent =
                            await rahmi.handleInteractive(content, jid, quoted);
                    ...
                    case "ALBUM":
                        const albumContent = await rahmi.handleAlbum(
                            content,
                            jid,
                            quoted
                        );
                    ...
                    case "POLL_RESULT":
                        return await rahmi.handlePollResult(
                            content,
                            jid,
                            quoted
                        );
            ...
            if (isDeleteMsg) {
                ...
                const isGroup = WABinary_1.isJidGroup(
                    content.delete?.remoteJid
                );
                additionalAttributes.edit =
                    (isGroup && !fromMe) || WABinary_1.isJidNewsLetter(jid)
                        ? "8"
                        : "7";
            } else if (isEditMsg) {
                additionalAttributes.edit = WABinary_1.isJidNewsLetter(jid)
                    ? "3"
                    : "1";
            } else if (isAiMsg) {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            /** i fix it / add it — github.com/balxz **/
